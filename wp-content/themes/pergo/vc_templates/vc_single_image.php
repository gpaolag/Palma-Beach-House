...
$caption = $img_link_large = $link = $img_link_target = $alignment = $el_class = $el_id = $css_animation = $style = $external_style = $border_color = $css = $icon_class = $external_img_alt = '';
...
switch ( $source ) {
	...
	case 'external_link':
		...
		$custom_alt = ($external_img_alt != '')? $external_img_alt : 'Image';

		$img = array(
			'thumbnail' => '<img class="vc_single_image-img img-fluid" ' . $hwstring . ' src="' . $custom_src . '" alt="'.esc_attr($custom_alt).'" />',
		);
		break;
...
switch ( $onclick ) {
	...
	case 'video':
		$link = pergo_validate_video_url($video_link);
		break;	
...
if ( $link ) {
	...
	$html = '<a ' . vc_stringify_attributes( $a_attrs ) . ' class="' . $wrapperClass . '">' .$video_icon. $img['thumbnail'] . '</a>';
} else {

        ...
        foreach( (array) $this->get_pages( $option ) as $page ) {
          ...
          if ( ! isset( $page['hidden_page'] ) && $screen->id == $this->page_hook[$page['id']] ) {
              ...
              foreach( $page['contextual_help']['content'] as $contextual_help ) {
                ...
                $screen->add_help_tab( 
                  ...
                  array(
                    ...
                    'content' => wpb_js_remove_wpautop( $contextual_help['content'] ),
            ...
            if ( ! empty( $page['contextual_help']['sidebar'] ) ) {
              ...
              $screen->set_help_sidebar( wpb_js_remove_wpautop( $page['contextual_help']['sidebar'] ) );
          ...
          if ( ! isset( $page['hidden_page'] ) && $screen->id == $this->page_hook[$page['id']] ) {
            ...
            if ( $show_buttons ) {
              ...
              if ( $show_buttons ) {
                ...
                echo '<form method="post" action="' . str_replace( '&settings-updated=true', '', admin_url( 'themes.php?page=ot-theme-options' )) . '">';                  

                  /* form nonce */
                  wp_nonce_field( 'option_tree_reset_form', 'option_tree_reset_nonce' );        

                  echo '<input type="hidden" name="action" value="reset" />';  
                  echo '<button type="submit" class="option-tree-ui-button button button-secondary left reset-settings" title="' . __( 'Reset Options', 'pergo' ) . '">' . __( 'Reset Options', 'pergo' ) . '</button>';
      ...
      foreach( (array) $this->options as $option ) {          
        ...
        foreach( (array) $this->get_pages( $option ) as $page ) {          
          ...
          foreach( (array) $this->get_the_settings( $page ) as $setting ) {            
            ...
            $section = isset($setting['section'])? $setting['section'] : 'general_options';            
 ...
 * @param    array    Array of arestructuraents to create settings

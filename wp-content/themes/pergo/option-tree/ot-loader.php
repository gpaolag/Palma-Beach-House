...
if ( class_exists( 'OT_Loader' ) && defined( 'OT_PLUGIN_MODE' ) && OT_PLUGIN_MODE == true ) { 
  ...
  function ot_conflict_notice() {  
    echo '<div class="error"><p>' . __( 'OptionTree is installed as a plugin and also embedded in your current theme. Please deactivate the plugin to load the theme dependent version of OptionTree, and remove this warning.', 'pergo' ) . '</p></div>';
  }
...
if ( ! class_exists( 'OT_Loader' ) ) {
  ...
  class OT_Loader {    
    ...
    public function load_textdomain() {    
      if ( OT_PLUGIN_MODE ) {
        load_plugin_textdomain( 'pergo', false, OT_LANG_DIR ); 
      } else {
        load_theme_textdomain( 'pergo', OT_LANG_DIR );       
    ...
    private function hooks() {      
      ...
      add_action( 'admin_bar_menu', 'ot_register_theme_options_adminbar_menu', 999 );
    ...
    public function change_image_button( $translation, $text, $domain ) {
      ...
      if ( $pagenow == apply_filters( 'ot_theme_options_parent_slug', 'themes.php' ) && 'default' == $domain && 'Insert into post' == $text ) {
        return apply_filters( 'ot_upload_text', __( 'Send to Option', 'pergo' ) );
      }      

...
if ( ! function_exists( 'crmm_admin_styles' ) ) {
  ...
  function crmm_admin_styles() {
    ...
    /* load admin styles */
    if(is_rtl()){
      wp_enqueue_style( 'crmm-admin-css', CRMM_URL . 'assets/css/crmm-admin.rtl.css', false, CRMM_VERSION );
    }else{
...
if ( ! function_exists( 'crmm_import' ) ) {
  ...
  function crmm_import() {
    ...
    if ( isset( $_POST['import_settings_nonce'] ) && wp_verify_nonce( $_POST['import_settings_nonce'], 'import_settings_form' ) ) {
      ...
      /* textarea value */
      $textarea = isset( $_POST['import_settings'] ) ? crmm_decode( $_POST['import_settings'] ) : '';
    ...
    if ( isset( $_POST['import_data_nonce'] ) && wp_verify_nonce( $_POST['import_data_nonce'], 'import_data_form' ) ) {
      ...
      /* textarea value */
      $options = isset( $_POST['import_data'] ) ? crmm_decode( $_POST['import_data'] ) : '';
    ...
    if ( isset( $_POST['import_layouts_nonce'] ) && wp_verify_nonce( $_POST['import_layouts_nonce'], 'import_layouts_form' ) ) {
      ...
      /* textarea value */
      $layouts = isset( $_POST['import_layouts'] ) ? crmm_decode( $_POST['import_layouts'] ) : '';
      ...
      if ( is_array( $layouts ) ) {
        ...
        if ( is_array( $settings ) ) {
          ...
          foreach( $layouts as $key => $value ) {
            ...
            $layouts[$key] = crmm_encode( $options );
        ...
        if ( isset( $layouts['active_layout'] ) ) {
          ...
          $new_options = crmm_decode( $layouts[$layouts['active_layout']] );
...
if ( ! function_exists( 'crmm_modify_layouts' ) ) {
  ...
  function crmm_modify_layouts() {
    ...
    if ( isset( $_POST['cool_megamenu_modify_layouts_nonce'] ) && wp_verify_nonce( $_POST['cool_megamenu_modify_layouts_nonce'], 'cool_megamenu_modify_layouts_form' ) ) {
      ...
      $megamenu_layouts = get_option( crmm_layouts_id() );
      ...
      if ( is_array( $layouts ) && ! empty( $layouts ) ) {
        ...
        if ( isset( $layouts['_add_new_layout_'] ) && ! empty( $layouts['_add_new_layout_'] ) ) {
          ...
          $rebuild[$rebuild['active_layout']] = crmm_encode( get_option( crmm_options_id(), [] ) );
        }
        ...
        foreach( $layouts as $key => $layout ) {
          ...
          if ( isset( $megamenu_layouts[$key] ) && ! empty( $megamenu_layouts[$key] ) ) {
            $rebuild[$key] = $megamenu_layouts[$key];
            if ( '' == $first_layout ) {
              $first_layout = $key;
            }
          }
      ...
      if ( count( $rebuild ) > 1 ) {
        ...
        $rebuild_cool_megamenu = crmm_decode( $rebuild[$rebuild['active_layout']] );

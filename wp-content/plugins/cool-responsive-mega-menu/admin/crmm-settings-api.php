...
/**
 * Cool Megamenu Settings API
 *
 * This class loads all the methods and helpers specific to a Settings page.
 *
 * @package   Cool Megamenu
 * @author    Razib <themeperch@gmail.com>
 * @copyright Copyright (c) 2018, Razib
 */
if ( ! class_exists( 'CRMM_Settings' ) ) {
  ...
  class CRMM_Settings {
    ...
    public function hooks() {
       ...
       */
      $priority = apply_filters( 'crmm_admin_menu_priority', 10 );
              ...
              foreach( $page['contextual_help']['content'] as $contextual_help ) {
                $screen->add_help_tab( 
                  array(
                    ...
                    'content' => isset($contextual_help['content'])? htmlspecialchars_decode( $contextual_help['content'] ) : '',
                  )
                );
              }
            }
          ...
          if ( ! isset( $page['hidden_page'] ) && $screen->id == $this->page_hook[$page['id']] ) {
            ...
            if ( isset( $_REQUEST['settings-updated'] ) && $_REQUEST['settings-updated'] == 'true' ) {
              ...
              if ( isset( $layouts['active_layout'] ) ) {
                $cool_megamenu = get_option( $option['id'] );
                $layouts[$layouts['active_layout']] = crmm_encode(  $cool_megamenu  );
            ...
            echo '<div class="wrap settings-wrap" id ="page-' . $page['id'] . '">';
              ...
              echo crmm_alert_message( $page );
              
              settings_errors( 'cool-megamenu' );
              
              /* Header */
              echo '<div id="crmm-header-wrap">';
              
                echo '<ul id="crmm-header">';
                  
                  echo '<li id="crmm-logo">' . apply_filters( 'crmm_header_logo_link', '<a class="theme-logo" href="http://wordpress.org/extend/plugins/cool-responsive-mega-menu/" target="_blank">Cool Megamenu</a>', $page['id'] ) . '</li>';
                  
                  echo '<li id="crmm-version"><span>' . apply_filters( 'crmm_header_version_text', CRMM_VERSION, $page['id'] ) . '</span></li>';
                  
                  // Add additional theme specific links here.
                  do_action( 'crmm_header_list', $page['id'] );
                
                echo '</ul>';
                
                /* layouts form */
                if ( CRMM_SHOW_NEW_LAYOUT == true ){
                  crmm_options_layouts_form();
                }
            ...
            if ( $show_buttons ) {
              ...
              echo '<form action="options.php" method="post" id="crmm-settings-api">';
  
                settings_fields( $option['id'] );
                
            } else {
              
              echo '<div id="crmm-settings-api">';
              
            }
                
                /* Sub Header */
                echo '<div id="crmm-sub-header">';
                  
                if ( $show_buttons )
                  echo '<button class="crmm-ui-button button button-primary right">' . $page['button_text'] . '</button>';
              ...
              if ( $show_buttons ) {
                ...
                echo '<div class="crmm-ui-buttons">';
              ...
              if ( $show_buttons ) {
                ...
                echo '<form method="post" action="' . str_replace( '&settings-updated=true', '', $_SERVER["REQUEST_URI"] ) . '">';
                  ...
                  /* form nonce */
                  wp_nonce_field( 'cool_megamenu_reset_form', 'cool_megamenu_reset_nonce' );
        
                  echo '<input type="hidden" name="action" value="reset" />';
                  
                  echo '<button type="submit" class="crmm-ui-button button button-secondary left crmm-reset-settings" title="' . __( 'Reset Options', 'cool-megamenu' ) . '">' . __( 'Reset Options', 'cool-megamenu' ) . '</button>';
    ...
    public function display_setting( $args = array() ) {
      ...
      /* set standard value */
      if ( isset( $std ) ) {  
        $field_value = crmm_filter_std_value( $field_value, $std );
      }
      
      // Allow the descriptions to be filtered before being displayed
      $desc = apply_filters( 'crmm_filter_description', ( isset( $desc ) ? $desc : '' ), $id );
      ...
      if ( $type == 'google-fonts' ) {
        ...
        crmm_set_google_fonts( $id, $field_value );
      }
        ...
        foreach( (array) $this->get_pages( $option ) as $page ) {
          ...
          foreach( (array) $this->get_the_settings( $page ) as $setting ) {
            ...
            if ( isset( $setting['std'] ) ) {
              ...
              $defaults[$setting['id']] = crmm_validate_setting( $setting['std'], $setting['type'], $setting['id'] );
        ...
        foreach( (array) $this->get_pages( $option ) as $page ) {
          ...
          foreach( (array) $this->get_the_settings( $page ) as $setting ) {
            ...
            if ( isset( $setting['type'] ) && isset( $input[$setting['id']] ) ) {
              ...
              /* get the defaults */
              $current_settings = get_option( crmm_settings_id() );
              ...
              if ( is_array( $input[$setting['id']] ) && in_array( $setting['type'], array( 'list-item', 'slider' ) ) ) {
                ...
                $settings = isset( $_POST[$setting['id'] . '_settings_array'] ) ? crmm_decode( $_POST[$setting['id'] . '_settings_array'] ) : array();
                
                /* settings are empty for some odd ass reason get the defaults */
                if ( empty( $settings ) ) {
                  $settings = 'slider' == $setting['type'] ? 
                  crmm_slider_settings( $setting['id'] ) : 
                  crmm_list_item_settings( $setting['id'] );
                }
                ...
                foreach( $input[$setting['id']] as $k => $setting_array ) {
                  ...
                  foreach( $settings as $sub_setting ) {
                    ...
                    if ( isset( $sub_setting['type'] ) && isset( $input[$setting['id']][$k][$sub_setting['id']] ) ) {
                      ...
                      /* validate setting */
                      $input[$setting['id']][$k][$sub_setting['id']] = crmm_validate_setting( $input[$setting['id']][$k][$sub_setting['id']], $sub_setting['type'], $sub_setting['id'], $wpml_id );
                ...
                if ( empty( $settings ) ) {
                  $settings = crmm_social_links_settings( $setting['id'] );
                }
                ...
                $input[$setting['id']] = crmm_validate_setting( $input[$setting['id']], $setting['type'], $setting['id'], $setting['id'] );
                  ...
                  foreach( $current_options[$setting['id']] as $key => $value ) {
                    ...
                    foreach( $value as $ckey => $cvalue ) {
                      ...
                      if ( ! in_array( $id, $wpml_ids ) ) {
                        ...
                        crmm_wpml_unregister_string( $id );
      ...
      foreach ( (array) $wp_settings_sections[$page] as $section ) {
        ...
        echo '<div id="section_' . $section_id . '" class="postbox ui-tabs-panel">';
          ...
          echo '<div class="inside">';
             ...
             */
            do_action( 'crmm_do_settings_fields_before', $page, $section_id );
             ...
             * @param string $section_id The section ID.
             */
            do_action( 'crmm_do_settings_fields_after', $page, $section_id );
    ...
    public function reset_options() {
      ...
      if ( isset( $_POST['cool_megamenu_reset_nonce'] ) && wp_verify_nonce( $_POST['cool_megamenu_reset_nonce'], 'cool_megamenu_reset_form' ) ) {
 ...
 * @since    2.0
 */
if ( ! function_exists( 'crmm_register_settings' ) ) {

  function crmm_register_settings( $args ) {
    if ( ! $args )
      return;
      
    $crmm_settings = new CRMM_Settings( $args );
  }

}

/* End of file crmm-settings-api.php */
/* Location: ./includes/crmm-settings-api.php */
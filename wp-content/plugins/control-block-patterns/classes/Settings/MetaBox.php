...
class MetaBox extends Meta_Box {
	...
	protected function object_hooks() {
		add_action( 'ctrlbp_settings_page_load', array( $this, 'load' ) );

		if ( $this->tab ) {
 			add_action( "ctrlbp_before_{$this->id}", array( $this, 'show_tab' ) );
 		}
	}
	...
	public function load( $page_args ) {
		...
		$data = apply_filters( 'ctrlbp_settings_pages_data', $data, $object_id );
	...
	public function show_tab() {
		echo '<script type="text/html" class="ctrlbp-settings-tab" data-tab="', esc_attr( $this->tab ), '"></script>';
	}
	...
	public function register_fields() {
		$registry = ctrlbp_get_registry( 'field' );		
		foreach ( $this->pages as $page ) {	
			foreach ( $this->meta_box['fields'] as &$field ) {
				$registry->add( $field, $page->option_name, $this->object_type );
				if ( isset( $field['type'] ) &&  in_array( $field['type'], ['backup', 'import']) ) {

<?php
namespace ControlPatterns\Term;
...
class MetaBox extends Meta_Box {
	...
	public function add() {
		...
		foreach ( $this->meta_box['taxonomies'] as $taxonomy ) {
			...
			add_action( "{$taxonomy}_term_edit_form_tag", array( 'ControlPatterns\Fields\File', 'post_edit_form_tag' ) );
		}
	}
	...
	public function enqueue() {
		...
		wp_enqueue_style( 'ctrlbp-term-meta', CTRLBP_CSS_URI . 'term-meta.css', '', CTRLBP_VER );
		...
		wp_enqueue_script( 'ctrlbp-term-meta', CTRLBP_JS_URI . 'term-meta.js', array( 'jquery' ), CTRLBP_VER, true );
		wp_localize_script( 'ctrlbp-term-meta', 'CTRLBPTermMeta', array(
			'addedMessage' => __( 'Term added.', 'control-block-patterns' ),
		) );
	}

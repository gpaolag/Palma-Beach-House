<?php
namespace ControlPatterns;
/**
 * Add about page for the Control Block Patterns plugin.
 *
 * @package ControlPatterns
 */

/**
 * About page class.
 */
class About {
	/**
	 * The updater checker object.
	 *
	 * @var object
	 */
	private $update_checker;

	/**
	 * Constructor.
	 *
	 * @param object $update_checker The updater checker object.
	 */
	public function __construct( $update_checker ) {
		$this->update_checker = $update_checker;
		$this->init();
	}

	/**
	 * Init hooks.
	 */
	public function init() {
		// Add links to about page in the plugin action links.
		add_filter( 'plugin_action_links_control-block-patterns/control-block-patterns.php', array( $this, 'plugin_links' ), 20 );

		// Add a shared top-level admin menu and Dashboard page. Use priority 5 to show Dashboard at the top.
		add_action( 'admin_menu', array( $this, 'add_menu' ), 5 );
		add_action( 'admin_menu', array( $this, 'add_submenu' ), 5 );

		// If no admin menu, then hide the About page.
		add_action( 'admin_head', array( $this, 'hide_page' ) );

		// Redirect to about page after activation.
		add_action( 'activated_plugin', array( $this, 'redirect' ), 10, 2 );
	}

	/**
	 * Add links to About page.
	 *
	 * @param array $links Array of plugin links.
	 *
	 * @return array
	 */
	public function plugin_links( $links ) {
		$links[] = '<a href="' . esc_url( $this->get_menu_link() ) . '">' . esc_html__( 'About', 'control-block-patterns' ) . '</a>';
		
		return $links;
	}

	/**
	 * Register admin page.
	 */
	public function add_menu() {
		if ( ! $this->has_menu() ) {
			return;
		}
		add_menu_page(
			__( 'Control Block Patterns', 'control-block-patterns' ),
			__( 'Control Block Patterns', 'control-block-patterns' ),
			'activate_plugins',
			'control-block-patterns',
			'__return_null',
			'data:image/svg+xml;base64,'
		);
	}

	/**
	 * Add submenu for the About page.
	 */
	public function add_submenu() {
		$parent_menu = $this->has_menu() ? 'control-block-patterns' : $this->get_parent_menu();
		$about       = add_submenu_page(
			$parent_menu,
			__( 'Welcome to Control Block Patterns', 'control-block-patterns' ),
			__( 'Dashboard', 'control-block-patterns' ),
			'activate_plugins',
			'control-block-patterns',
			array( $this, 'render' )
		);
		add_action( "load-$about", array( $this, 'load_about' ) );
	}

	/**
	 * Functions and hooks for about page.
	 */
	public function load_about() {
		$this->enqueue();
		add_filter( 'admin_footer_text', array( $this, 'change_footer_text' ) );
	}

	/**
	 * Hide about page from the admin menu.
	 */
	public function hide_page() {
		remove_submenu_page( $this->get_parent_menu(), 'control-block-patterns' );
	}

	/**
	 * Render admin page.
	 */
	public function render() {
		?>
		<div class="wrap">
			<div id="poststuff">
				<div id="post-body" class="metabox-holder columns-2">
					<div id="post-body-content">
						<div class="about-wrap">
							<?php
							include __DIR__ . '/sections/welcome.php';
							include __DIR__ . '/sections/tabs.php';							
							include __DIR__ . '/sections/getting-started.php';							
							include __DIR__ . '/sections/support.php';
							do_action( 'ctrlbp_about_tabs_content' );
							?>
						</div>
					</div>
					<div id="postbox-container-1" class="postbox-container">
						<?php
						include __DIR__ . '/sections/products.php';						
						?>
					</div>
				</div>
			</div>
		</div>
		<?php
	}

	/**
	 * Enqueue CSS and JS.
	 */
	public function enqueue() {
		wp_enqueue_style( 'control-block-patterns-about', CTRLBP_URI . 'assets/about/css/about.css', array(), CTRLBP_VER );
		wp_enqueue_script( 'control-block-patterns-about', CTRLBP_URI . 'assets/about/js/about.js', array( 'jquery' ), CTRLBP_VER, true );
	}

	/**
	 * Change WordPress footer text on about page.
	 */
	public function change_footer_text() {
		// Translators: %1$s - link to review form.
		echo wp_kses_post( sprintf( __( 'Please rate <strong>Control Block Patterns</strong> <a href="%1$s" target="_blank">&#9733;&#9733;&#9733;&#9733;&#9733;</a> on <a href="%1$s" target="_blank">WordPress.org</a> to help us spread the word. Thank you from the SenseFlame!', 'control-block-patterns' ), 'https://wordpress.org/support/view/plugin-reviews/control-block-patterns?filter=5#new-post' ) );
	}

	/**
	 * Redirect to about page after Control Block Patterns has been activated.
	 *
	 * @param string $plugin       Path to the main plugin file from plugins directory.
	 * @param bool   $network_wide Whether to enable the plugin for all sites in the network
	 *                             or just the current site. Multisite only. Default is false.
	 */
	public function redirect( $plugin, $network_wide = false ) {
		$is_cli           = 'cli' === php_sapi_name();
		$is_plugin        = 'control-block-patterns/control-block-patterns.php' === $plugin;
		$is_bulk_activate = 'activate-selected' === ctrlbp_request()->post( 'action' ) && count( ctrlbp_request()->post( 'checked' ) ) > 1;

		if ( ! $is_plugin || $network_wide || $is_cli || $is_bulk_activate || $this->is_bundled() ) {
			return;
		}
		wp_safe_redirect( $this->get_menu_link() );
		die;
	}

	/**
	 * Get link to the plugin admin menu.
	 *
	 * @return string
	 */
	protected function get_menu_link() {
		$menu = $this->has_menu() ? 'admin.php?page=control-block-patterns' : $this->get_parent_menu() . '?page=control-block-patterns';
		return admin_url( $menu );
	}

	/**
	 * Get default parent menu, which is Plugins.
	 *
	 * @return string
	 */
	protected function get_parent_menu() {
		return 'plugins.php';
	}

	/**
	 * Check if the plugin has a top-level admin menu.
	 *
	 * @return bool
	 */
	protected function has_menu() {
		return apply_filters( 'ctrlbp_admin_menu', false );
	}

	/**
	 * Check if Control Block Patterns is bundled by TGM Activation Class.
	 *
	 * @return bool
	 */
	protected function is_bundled() {
		// @codingStandardsIgnoreLine
		foreach ( $_REQUEST as $key => $value ) {
			if ( false !== strpos( $key, 'tgmpa' ) || ( ! is_array( $value ) && false !== strpos( $value, 'tgmpa' ) ) ) {
				return true;
			}
		}
		return false;
	}
}

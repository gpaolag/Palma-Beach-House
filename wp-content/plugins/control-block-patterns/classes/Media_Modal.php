...
class Media_Modal {
	...
	public function init() {
		...
		add_filter( 'ctrlbp_show', array( $this, 'is_in_normal_mode' ), 10, 2 );
	}
	...
	public function add_fields( $form_fields, \WP_Post $post ) {
		foreach ( $this->fields as $field ) {
			...
			$meta                = CTRLBP_Field::call( $field, 'meta', $post->ID, true );
			...
			CTRLBP_Field::call( 'show', $field, false );
	...
	public function save_fields( $post, $attachment ) {
		foreach ( $this->fields as $field ) {
			...
			$old = Field::call( $field, 'raw_meta', $post['ID'] );
			$new = isset( $attachment[ $key ] ) ? $attachment[ $key ] : '';

			$new = Field::process_value( $new, $post['ID'], $field );

			// Call defined method to save meta value, if there's no methods, call common one.
			Field::call( $field, 'save', $new, $old, $post['ID'] );
		}
	 ...
	 * @param array $meta_box Control Block Patterns parameters.
	 *
	 * @return bool
	 */

...
use ControlPatterns\Meta_Box;

class Block extends Meta_Box {
	protected $storage;
	...
	protected function object_hooks() {
		...
		add_action( 'wp_ajax_ctrlbp_blocks_fetch', [ $this, 'fetch' ] );
		add_action( 'wp_ajax_ctrlbp_blocks_save', [ $this, 'save_block' ] );
	}
	...
	private function add_block_data() {
		...
		wp_add_inline_script( 'ctrlbp-blocks', "ctrlbp.blocks.push({$json});", 'before' );
	}

	private function register_block_type() {
		register_block_type( "ctrlbp/{$this->id}", [
			'editor_script'   => 'ctrlbp-blocks',
			'editor_style'    => 'ctrlbp-blocks',
	...
	public function fetch() {
		...
		if ( "ctrlbp/$this->id" !== $request->post( 'block' ) ) {
			return;
		}
	...
	private function preview( $attributes = [] ) {
		...
		$post_id = ctrlbp_request()->post( 'post_id' );
	...
	protected function render_block( $attributes = [], $is_preview = false, $post_id = null ) {
		...
		if ( file_exists( $this->render_template ) ) {
			...
			$args = array_merge($data, $attributes);
			locate_template( $this->render_template, true, true, $args );
		}
	}

	private function enqueue_block_assests() {
		$handle = "ctrlbp/$this->id";

...
class Map extends Field {
	...
	public static function admin_enqueue_scripts() {
		 ...
		 */
		$google_maps_url = apply_filters( 'ctrlbp_google_maps_url', $google_maps_url );
		wp_register_script( 'google-maps', esc_url_raw( $google_maps_url ), array(), CTRLBP_VER, true );
		wp_enqueue_script(
			'ctrlbp-map',
			CTRLBP_JS_URI . 'map.js',
			array(
		...
		CTRLBP_Helpers_Field::localize_script_once(
			'ctrlbp-map',
			'CTRLBP_Map',
			array(
				'no_results_string' => __( 'No results found', 'control-block-patterns' ),
			)
		);
	}
	...
	public static function html( $meta, $field ) {
		...
		$html    = sprintf(
			'<div class="ctrlbp-map-field" data-address-field="%s">',
			esc_attr( $address )
		);
		...
		$html .= sprintf(
			'<div class="ctrlbp-map-canvas" data-default-loc="%s" data-region="%s"></div>
			<input %s>',
			esc_attr( $field['std'] ),
	...
	public static function render_map( $location, $args = array() ) {
		...
		$google_maps_url = apply_filters( 'ctrlbp_google_maps_url', $google_maps_url );
		wp_register_script( 'google-maps', esc_url_raw( $google_maps_url ), array(), CTRLBP_VER, true );
		wp_enqueue_script( 'ctrlbp-map-frontend', CTRLBP_JS_URI . 'map-frontend.js', array( 'google-maps', 'jquery' ), CTRLBP_VER, true );
		...
		$output = sprintf(
			'<div class="ctrlbp-map-canvas" data-map_options="%s" style="width:%s;height:%s"></div>',

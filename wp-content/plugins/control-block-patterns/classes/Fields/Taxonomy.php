 ...
 */
class Taxonomy extends Object_Choice {
	/**
	 * Add ajax actions callback.
	 */
	public static function add_actions() {
		add_action( 'wp_ajax_ctrlbp_get_terms', array( __CLASS__, 'ajax_get_terms' ) );
		add_action( 'wp_ajax_nopriv_ctrlbp_get_terms', array( __CLASS__, 'ajax_get_terms' ) );
	}
	...
	public static function normalize( $field ) {
		 ...
		 */
		$placeholder   = __( 'Select a term', 'control-block-patterns' );
		$taxonomy_name = self::get_taxonomy_singular_name( $field );
		if ( $taxonomy_name ) {
			// Translators: %s is the taxonomy singular label.
			$placeholder = sprintf( __( 'Select a %s', 'control-block-patterns' ), strtolower( $taxonomy_name ) );
		}
		$field = wp_parse_args(
			$field,
			array(
	...
	public static function query( $meta, $field ) {
		...
		foreach ( $terms as $term ) {
			$label = $term->name ? $term->name : __( '(No title)', 'control-block-patterns' );
	...
	protected static function add_term( $field ) {
		$term = ctrlbp_request()->post( $field['id'] . '_new' );
	...
	public static function add_new_form( $field ) {
		...
		$html = '
		<div class="ctrlbp-taxonomy-add">
			<button class="ctrlbp-taxonomy-add-button">%s</button>
			<div class="ctrlbp-taxonomy-add-form ctrlbp-hidden">
	...
	public static function admin_enqueue_scripts() {
		...
		wp_enqueue_script( 'ctrlbp-taxonomy', CTRLBP_JS_URI . 'taxonomy.js', array( 'jquery' ), CTRLBP_VER, true );

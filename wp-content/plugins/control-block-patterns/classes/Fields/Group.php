...
use ControlPatterns\Cloner as Cloner;
use ControlPatterns\Meta_Box as Meta_Box;
use ControlPatterns\Meta_Group as Meta_Group;
...
class Group extends Field {
	...
	public static function admin_enqueue_scripts() {
		...
		wp_enqueue_script( 'ctrlbp-group', CTRLBP_JS_URI . 'group.js', array( 'jquery', 'underscore' ), CTRLBP_VER, true );
		wp_localize_script(
			'ctrlbp-group',
			'CTRLBP_Group',
			array(
				'confirmRemove' => __( 'Are you sure you want to remove this group?', 'control-block-patterns' ),
			)
		);
	}
	...
	public static function html( $meta, $field ) {
		...
		if ( empty( self::$meta_queue ) ) {
			add_filter( 'ctrlbp_raw_meta', array( __CLASS__, 'child_field_meta' ), 10, 3 );
		}
		...
		foreach ( $field['fields'] as $child_field ) {
			...
			self::call( 'show', $child_field, Meta_Group::$saved );
		}
		...
		if ( empty( self::$meta_queue ) ) {
			remove_filter( 'ctrlbp_raw_meta', array( __CLASS__, 'child_field_meta' ) );
		}
	...
	protected static function output_collapsible_elements( $field, $meta ) {
		...
		$remove_button = '';
		if ( $field['clone'] ) {
			$remove_button = sprintf('<a href="javascript:;" class="ctrlbp-group-remove">%s</a>', esc_html__( 'Remove', 'control-block-patterns' ));
		}

		// Collapse/expand icon.
		$expand_button = '<button aria-expanded="true" class="ctrlbp-group-toggle-handle button-link"><span class="ctrlbp-group-toggle-indicator" aria-hidden="true"></span></button>';

		
		echo '<div class="ctrlbp-group-title-wrapper">';
		echo '<h4 ', self::render_attributes( $title_attributes ), '>', $title, '</h4>';
		echo $remove_button, $expand_button;
	...
	protected static function get_sub_values( $fields, $new, $post_id ) {
		...
		foreach ( $fields as $field ) {
			...
			if ( 'group' === $field['type'] ) {
				$value               = $field['clone'] ? Cloner::value( $value, array(), $post_id, $field ) : self::get_sub_values( $field['fields'], $value, $post_id );
				$new[ $field['id'] ] = $value;
				continue;
			}
			...
			// File uploads.
			if ( $field['clone'] ) {
				$value = File::clone_value( $value, array(), $post_id, $field, $new );
			} else {
				$index          = isset( $new[ "_index_{$field['id']}" ] ) ? $new[ "_index_{$field['id']}" ] : null;
				$field['index'] = $index;
				$value          = File::value( $value, '', $post_id, $field );
			}
	...
	public static function normalize( $field ) {
		...
		$field['fields'] = empty( $field['fields'] ) ? [] : Meta_Box::normalize_fields( $field['fields'] );

		$field = wp_parse_args( $field, [
			'collapsible'   => true,
			'save_state'    => true,
			'group_title'   => $field['clone'] ? __( 'Group Item {#}', 'control-block-patterns' ) : __( 'Group Item', 'control-block-patterns' ),
			'default_state' => 'collapsed',
		] );

		if ( $field['collapsible'] ) {
			$field['class'] .= ' ctrlbp-group-collapsible';

			if ( 'collapsed' === $field['default_state'] ) {
				$field['class'] .= ' ctrlbp-group-collapsed';
			}
		}
		// Add a new hidden field to save the collapse/expand state.
		if ( $field['save_state'] ) {
			$field['fields'][] = Input::normalize(
				array(
					'type'       => 'hidden',
					'id'         => '_state',
					'std'        => $field['default_state'],
					'class'      => 'ctrlbp-group-state',
					'attributes' => array(
		...
		if ( ! $field['clone'] ) {
			$field['class'] .= ' ctrlbp-group-non-cloneable';
		}

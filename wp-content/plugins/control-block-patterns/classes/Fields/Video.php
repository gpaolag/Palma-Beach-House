...
class Video extends Media {
	...
	public static function admin_enqueue_scripts() {
		...
		wp_enqueue_script( 'ctrlbp-video', CTRLBP_JS_URI . 'video.js', array( 'ctrlbp-media' ), CTRLBP_VER, true );
	...
	public static function file_info( $file_id, $args = array(), $field = array() ) {
		...
		$thuctrlbp_id = get_post_thumbnail_id( $attachment->ID );
		if ( ! empty( $thuctrlbp_id ) ) {
			list( $src, $width, $height ) = wp_get_attachment_image_src( $thuctrlbp_id, 'full' );
			$data['image']                = compact( 'src', 'width', 'height' );
			list( $src, $width, $height ) = wp_get_attachment_image_src( $thuctrlbp_id, 'thumbnail' );
	...
	public static function print_templates() {
		...
		require_once CTRLBP_INC_DIR . 'templates/video.php';
	}
}

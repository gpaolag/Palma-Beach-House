...
/**
 * The google_fonts field which uses HTML <input type="text">.
 *
 * @package ControlPatterns
 */

/**
 * Google_Fonts field class.
 */
class Google_Fonts extends Field {
	...
	public static function html( $meta, $field ) {
		...
		$font = Input::normalize(
			array(
				'type'        => 'text',
				'id'          => "{$field['id']}_font",
				'field_name'  => "{$field['field_name']}[font]",
				'clone'       => false,
				'system_fonts' => $field['system_fonts'],
				'google_fonts' => $field['google_fonts'],
				'attributes'  => [
					'role' => 'ctrlbp-font-family',
					'class' => 'ctrlbp-google-fonts',
				]
			)
		);
		$output .= Input::html($meta['font'], $font);
		
		$output .= '</div>';

		$output .= '<script>
			jQuery("#'. $field['id'] .'_font").fontselect();
		</script>';
	...
	public static function admin_enqueue_scripts() {
		...
		// wp_enqueue_style( 'ctrlbp-jquery-fontselect-style', CTRLBP_CSS_URI . 'google-fonts/jquery.fontselect.css');

		wp_enqueue_script( 'ctrlbp-jquery-fontselect',  CTRLBP_JS_URI. 'google-fonts/jquery.fontselect.js', array('jquery'), CTRLBP_VER , false );
		wp_localize_script( 'ctrlbp-jquery-fontselect',  'fonts',
			array(
				'system_fonts' => array_merge($field['system_fonts'], [
					'Theme Default', 'Arial', 'Helvetica+Neue', 'Courier+New', 'Times+New+Roman', 'Comic+Sans+MS', 'Verdana', 'Impact'
				]),
				'google_Fonts' => array_merge($field['google_fonts'],[

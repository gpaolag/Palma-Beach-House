...
class File extends Field {
	...
	public static function admin_enqueue_scripts() {
		wp_enqueue_script( 'ctrlbp-file', CTRLBP_JS_URI . 'file.js', array( 'jquery-ui-sortable' ), CTRLBP_VER, true );
		...
		Helpers_Field::localize_script_once(
			...
			array(
				...
				'maxFileUploadsSingle' => __( 'You may only upload maximum %d file', 'control-block-patterns' ),
				// Translators: %d is the number of files in plural form.
				'maxFileUploadsPlural' => __( 'You may only upload maximum %d files', 'control-block-patterns' ),
			)
		);
	}
	...
	public static function add_actions() {
		...
		add_action( 'wp_ajax_ctrlbp_delete_file', array( __CLASS__, 'ajax_delete_file' ) );
	}
	...
	public static function ajax_delete_file() {
		...
		check_ajax_referer( "ctrlbp-delete-file_{$field_id}" );
		...
		$field = ctrlbp_get_field_settings( $field_id, array( 'object_type' => $object_type ), $object_id );
		...
		if ( ( 'child' !== $type && ! in_array( $attachment, $field_value ) ) ||
			...
			wp_send_json_error( __( 'Error: Invalid file', 'control-block-patterns' ) );
		}
		// Delete the file.
		...
		wp_send_json_error( __( 'Error: Cannot delete file', 'control-block-patterns' ) );
	}
	...
	public static function html( $meta, $field ) {
		...
		$i18n_more = apply_filters( 'ctrlbp_file_add_string', _x( '+ Add new file', 'file upload', 'control-block-patterns' ), $field );
		...
		$attributes['class'] = 'ctrlbp-file-input';
		...
		// Upload new files.
		$html .= sprintf(
			'<div class="ctrlbp-file-new"><input %s>',
			self::render_attributes( $attributes )
		);
		if ( 1 !== $field['max_file_uploads'] ) {
			$html .= sprintf(
				'<a class="ctrlbp-file-add" href="#"><strong>%s</strong></a>',
				$i18n_more
			);
		}
		$html .= '</div>';
		...
		$html .= sprintf(
			'<input type="hidden" class="ctrlbp-file-index" name="%s" value="%s">',
			$field['index_name'],
			$field['input_name']
		);
	...
	protected static function get_uploaded_files( $files, $field ) {
		$delete_nonce  = wp_create_nonce( "ctrlbp-delete-file_{$field['id']}" );
		$output        = '';
		...
		return sprintf(
			'<ul class="ctrlbp-files" data-field_id="%s" data-field_name="%s" data-delete_nonce="%s" data-force_delete="%s" data-max_file_uploads="%s" data-mime_type="%s">%s</ul>',
			$field['id'],
			$field['field_name'],
			$delete_nonce,
	...
	protected static function file_html( $file, $index, $field ) {
		$i18n_delete = apply_filters( 'ctrlbp_file_delete_string', _x( 'Delete', 'file upload', 'control-block-patterns' ) );
		$i18n_edit   = apply_filters( 'ctrlbp_file_edit_string', _x( 'Edit', 'file upload', 'control-block-patterns' ) );
		...
		if ( $field['upload_dir'] ) {
			...
			if ( $edit_link ) {
				$data['edit_link'] = sprintf( '<a href="%s" class="ctrlbp-file-edit" target="_blank">%s</a>', $edit_link, $i18n_edit );
			}
		}

		return sprintf(
			'<li class="ctrlbp-file">
				<div class="ctrlbp-file-icon">%s</div>
				<div class="ctrlbp-file-info">
					<a href="%s" target="_blank" class="ctrlbp-file-title">%s</a>
					<div class="ctrlbp-file-name">%s</div>
					<div class="ctrlbp-file-actions">
						%s
						<a href="#" class="ctrlbp-file-delete" data-attachment_id="%s">%s</a>
					</div>
				</div>

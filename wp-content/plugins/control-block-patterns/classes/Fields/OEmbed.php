...
class OEmbed extends Input {
	...
	public static function normalize( $field ) {
		...
		$field               = wp_parse_args(
			$field,
			array(
				'not_available_string' => __( 'Embed HTML not available.', 'control-block-patterns' ),
			)
		);
	...
	public static function admin_enqueue_scripts() {
		wp_enqueue_script( 'ctrlbp-oembed', CTRLBP_JS_URI . 'oembed.js', array( 'jquery', 'underscore' ), CTRLBP_VER, true );
	}
	...
	public static function add_actions() {
		add_action( 'wp_ajax_ctrlbp_get_embed', array( __CLASS__, 'wp_ajax_get_embed' ) );
	}
	...
	public static function get_embed( $url, $not_available = '' ) {
		 ...
		 *
		 * @link https://github.com/rilwis/control-block-patterns/issues/801
		 * @see  WP_oEmbed::fetch()
		...
		if ( $not_available ) {
			$not_available = '<div class="ctrlbp-oembed-not-available">' . wp_kses_post( $not_available ) . '</div>';
		}
		$not_available = apply_filters( 'ctrlbp_oembed_not_available_string', $not_available );
	...
	public static function html( $meta, $field ) {
		return parent::html( $meta, $field ) . sprintf(
			...
			<div class="ctrlbp-embed-media">%s</div>',

 ...
 * Post field class.
 */
class Post extends Object_Choice {
	/**
	 * Add ajax actions callback.
	 */
	public static function add_actions() {
		add_action( 'wp_ajax_ctrlbp_get_posts', array( __CLASS__, 'ajax_get_posts' ) );
		add_action( 'wp_ajax_nopriv_ctrlbp_get_posts', array( __CLASS__, 'ajax_get_posts' ) );
	}
	...
	public static function normalize( $field ) {
		 ...
		 */
		$placeholder = __( 'Select a post', 'control-block-patterns' );
		if ( 1 === count( $field['post_type'] ) ) {
			$post_type        = reset( $field['post_type'] );
			$post_type_object = get_post_type_object( $post_type );
			if ( ! empty( $post_type_object ) ) {
				// Translators: %s is the taxonomy singular label.
				$placeholder = sprintf( __( 'Select a %s', 'control-block-patterns' ), strtolower( $post_type_object->labels->singular_name ) );
			}
		}
		$field = wp_parse_args(
			$field,
	...
	public static function query( $meta, $field ) {
		...
		$options      = wp_cache_get( $cache_key, 'control-block-patterns-post-field' );
		...
		foreach ( $query->posts as $post ) {
			$label = $post->post_title ? $post->post_title : __( '(No title)', 'control-block-patterns' );
		...
		// Cache the query.
		wp_cache_set( $cache_key, $options, 'control-block-patterns-post-field' );

...
class Datetime extends Input {
	...
	public static function register_assets() {
		...
		wp_register_style( 'jquery-ui-timepicker', "$url/jquery-ui-timepicker-addon.min.css", ['jquery-ui-slider'], '1.6.3' );

		//wp_register_style( 'ctrlbp-date', CTRLBP_CSS_URI . 'date.css', ['jquery-ui-datepicker'], CTRLBP_VER );

		// Scripts.
		$url = CTRLBP_JS_URI . 'jqueryui';
		...
		wp_register_script( 'ctrlbp-datetime', CTRLBP_JS_URI . 'datetime.js', ['jquery-ui-datepicker', 'jquery-ui-timepicker-i18n', 'underscore', 'jquery-ui-button', 'jquery-ui-timepicker-slider'], CTRLBP_VER, true );
		wp_register_script( 'ctrlbp-date', CTRLBP_JS_URI . 'date.js', ['jquery-ui-datepicker', 'underscore'], CTRLBP_VER, true );
		wp_register_script( 'ctrlbp-time', CTRLBP_JS_URI . 'time.js', ['jquery-ui-timepicker-i18n', 'jquery-ui-button', 'jquery-ui-timepicker-slider'], CTRLBP_VER, true );
		...
		foreach ( $handles as $handle ) {
			Helpers_Field::localize_script_once( "ctrlbp-$handle", 'CTRLBP_' . ucfirst( $handle ), $data );
		}
	}
	...
	public static function admin_enqueue_scripts() {
		...
		wp_enqueue_script( 'ctrlbp-datetime' );
	}
	...
	public static function html( $meta, $field ) {
		...
		if ( $field['timestamp'] ) {
			...
			$output .= sprintf(
				'<input type="hidden" name="%s" class="ctrlbp-datetime-timestamp" value="%s">',
		...
		if ( $field['inline'] ) {
			$output .= '<div class="ctrlbp-datetime-inline"></div>';
		}
	...
	public static function meta( $post_id, $saved, $field ) {
		...
		if ( $field['timestamp'] ) {
			return Helpers_Array::map( $meta, __CLASS__ . '::from_timestamp', $field );
		}

		if ( $field['save_format'] && $meta ) {
			return Helpers_Array::map( $meta, __CLASS__ . '::from_save_format', $field );
		}

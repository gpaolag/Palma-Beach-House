...
class OSM extends Field {
	...
	public static function admin_enqueue_scripts() {
		...
		wp_enqueue_script( 'ctrlbp-osm', CTRLBP_JS_URI . 'osm.js', array( 'jquery', 'jquery-ui-autocomplete', 'leaflet' ), CTRLBP_VER, true );
	...
	public static function html( $meta, $field ) {
		...
		$html    = sprintf(
			'<div class="ctrlbp-osm-field" data-address-field="%s">',
			esc_attr( $address )
		);
		...
		$html .= sprintf(
			'<div class="ctrlbp-osm-canvas" data-default-loc="%s" data-region="%s" data-language="%s"></div>
			<input %s>',
			esc_attr( $field['std'] ),
	...
	public static function render_map( $location, $args = array() ) {
		...
		wp_enqueue_style( 'leaflet', 'https://unpkg.com/leaflet@1.5.1/dist/leaflet.css', array(), '1.5.1' );
		wp_enqueue_script( 'leaflet', 'https://unpkg.com/leaflet@1.5.1/dist/leaflet.js', array(), '1.5.1', true );
		wp_enqueue_script( 'ctrlbp-osm-frontend', CTRLBP_JS_URI . 'osm-frontend.js', array( 'jquery', 'leaflet' ), CTRLBP_VER, true );
		...
		$output = sprintf(
			'<div class="ctrlbp-osm-canvas" data-osm_options="%s" style="width:%s;height:%s"></div>',

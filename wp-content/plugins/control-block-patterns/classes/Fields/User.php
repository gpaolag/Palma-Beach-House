 ...
 * User field class.
 */
class User extends Object_Choice {
	/**
	 * Add actions.
	 */
	public static function add_actions() {
		add_action( 'wp_ajax_ctrlbp_get_users', array( __CLASS__, 'ajax_get_users' ) );
		add_action( 'wp_ajax_nopriv_ctrlbp_get_users', array( __CLASS__, 'ajax_get_users' ) );
	...
	public static function normalize( $field ) {
		...
		$field = wp_parse_args(
			$field,
			array(
				'placeholder'   => __( 'Select an user', 'control-block-patterns' ),
				'query_args'    => array(),
	...
	public static function query( $meta, $field ) {
		...
		$options      = wp_cache_get( $cache_key, 'control-block-patterns-user-field' );
		if ( false !== $options ) {
			return $options;
		}
		...
		foreach ( $users as $user ) {
			$label = $user->$display_field ? $user->$display_field : __( '(No title)', 'control-block-patterns' );
		...
		// Cache the query.
		wp_cache_set( $cache_key, $options, 'control-block-patterns-user-field' );

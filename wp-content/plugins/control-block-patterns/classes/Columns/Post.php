...
class Post extends Base {
	...
	public function show( $column, $post_id ) {
		...
		$value = ctrlbp_meta( $field['id'], '', $post_id );
		...
		printf(
			'<div class="ctrlbp-admin-columns ctrlbp-admin-columns-%s" id="ctrlbp-admin-columns-%s">%s</div>',
			esc_attr( $field['type'] ),
			esc_attr( $field['id'] ),
			$config['before'] . ctrlbp_the_value( $field['id'], '', $post_id, false ) . $config['after']
		);
	}
	...
	private function search_post_ids_by_tables() {
		if ( ! class_exists( 'CTRLBP_Custom_Table_API' ) ) {
			return array();
		}
	...
	protected function output_filter_for( $taxonomy ) {
		wp_dropdown_categories( array(
			'show_option_all' => sprintf( __( 'All %s', 'ctrlbp-admin-columns' ), $taxonomy->label ),
			'orderby'         => 'name',
			'order'           => 'ASC',
			'hide_empty'      => false,
			'hide_if_empty'   => true,

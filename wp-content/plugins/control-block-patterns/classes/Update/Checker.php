...
/**
 * The main update logic for updating Control Block Patterns extensions.
 * The update checker class for Control Block Patterns extensions
 *
...
class Checker {
	 ...
	 * @var string
	 */
	private $api_url = 'https://controlpatterns.net/api/index.php';
	...
	public function check_updates( $data ) {
		...
		foreach ( $plugins as $plugin ) {
			if ( empty( $plugin->package ) ) {
				$plugin->upgrade_notice = __( 'UPDATE UNAVAILABLE! Please enter a valid license key to enable automatic updates.', 'control-block-patterns' );
			}

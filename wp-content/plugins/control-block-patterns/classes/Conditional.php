...
class Conditional {
	...
	public function __construct() {
		add_action( 'ctrlbp_before', array( $this, 'insert_meta_box_conditions' ) );
		add_action( 'ctrlbp_before', array( $this, 'insert_toggle_type' ) );


		add_filter( 'ctrlbp_wrapper_html', array( $this, 'insert_field_conditions' ), 10, 2 );
		add_action( 'ctrlbp_after', array( $this, 'enqueue_in_footer' ) );

		add_action( 'ctrlbp_enqueue_scripts', [ $this, 'enqueue_in_customizer_gutenberg' ] );
	}
	...
	public function insert_toggle_type( $obj ) {
		if ( $obj->toggle_type ) {
			echo '<script type="html/template" class="ctrlbp-toggle-type" data-toggle_type="' . esc_attr( $obj->toggle_type ) . '"></script>';
		}
	}
	...
	private function get_conditions_html( $settings ) {
		...
		return '<script type="html/template" class="ctrlbp-conditions" data-conditions="' . esc_attr( wp_json_encode( $conditions ) ) . '"></script>';
	}
	...
	public function enqueue_in_customizer_gutenberg() {
		...
		// We can't enqueue with "ctrlbp_after", and must use "ctrlbp_enqueue_scripts".
	...
	public function enqueue() {		
		wp_enqueue_script( 'ctrlbp-conditional', CTRLBP_JS_URI . 'conditional.js', ['jquery', 'underscore'], CTRLBP_VER, true );
		wp_localize_script( 'ctrlbp-conditional', 'conditions', $this->get_outside_conditions() );
	}
	...
	private function get_outside_conditions() {
		...
		$this->outside_conditions = apply_filters( 'ctrlbp_outside_conditions', [] );

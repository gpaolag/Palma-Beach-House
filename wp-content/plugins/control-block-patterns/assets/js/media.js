( function ( $, wp, _, ctrlbp, i18n ) {
	'use strict';

	var views = ctrlbp.views = ctrlbp.views || {},
		models = ctrlbp.models = ctrlbp.models || {},
		media = wp.media,
	...
	MediaCollection = Backbone.Collection.extend( {
		...
		remove: function ( models, options ) {
			...
			if( ! $( event.target ).closest( '.ctrlbp-field, [data-class="ctrlbp-field"]' ).length ) {
				return;
			}
	...
	MediaField = views.MediaField = Backbone.View.extend( {
		...
		initialize: function ( options ) {
			...
			collection.on( 'add remove reset', _.debounce( function () {
				...
				that.$input.val( ids ).trigger( 'change', [that.$( '.ctrlbp-media-input' )] );
			}, 500 ) );
		},
	...
	MediaStatus = views.MediaStatus = Backbone.View.extend( {
		tagName: 'div',
		className: 'ctrlbp-media-status',
		template: wp.template( 'ctrlbp-media-status' ),
	...
	MediaButton = views.MediaButton = Backbone.View.extend( {
		tagName: 'div',
		className: 'ctrlbp-media-add',
		template: wp.template( 'ctrlbp-media-button' ),
		events: {
	...
	MediaItem = views.MediaItem = Backbone.View.extend( {
		tagName: 'li',
		className: 'ctrlbp-file',
		template: wp.template( 'ctrlbp-media-item' ),
		initialize: function ( options ) {
			this.controller = options.controller;
			this.collection = this.controller.get( 'items' );
			this.render();
		...
		events: {
			'click .ctrlbp-image-overlay': function ( e ) {
				e.preventDefault();
				this.trigger( 'click:switch', this.model );
			},
			'click .ctrlbp-remove-media': function ( e ) {
				e.preventDefault();
				this.trigger( 'click:remove', this.model );
			},
			'click .ctrlbp-edit-media': function ( e ) {
				e.preventDefault();
	...
	MediaSelect = views.MediaSelect = MediaFrame.Select.extend( {
		...
		createStates: function () {
			...
			this.$el.attr( 'data-class', 'ctrlbp-field' );
	...
	function init( e ) {
		$( e.target ).find( '.ctrlbp-file_advanced' ).each( initMediaField );
	}

	ctrlbp.$document
		.on( 'ctrlbp_ready', init )
		.on( 'clone', '.ctrlbp-file_advanced', initMediaField );
} )( jQuery, wp, _, ctrlbp, i18nCtrlbpMedia );

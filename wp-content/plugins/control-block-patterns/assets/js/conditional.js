( function ( $, window, document, wp, api ) {
	...
	var conditions = window.conditions,
		inputSelectors = 'input[class*="ctrlbp"], textarea[class*="ctrlbp"], select[class*="ctrlbp"], button[class*="ctrlbp"]';
	...
	function runConditionalLogic( $scope ) {
		...
		var selectorCache = getSelectorCache( $scope ),
			$conditions = selectorCache.get( '.ctrlbp-conditions' );
		...
		$conditions.each( function () {
			...
			if ( !$element.hasClass( 'ctrlbp-field' ) ) {
	...
	function isLogicCorrect( logics, $field ) {
		...
		logics.when.forEach( function ( logic ) {
			...
			if ( !isGutenbergElement( logic[ 0 ] ) && !dependentFieldSelector && $scope && $scope.hasClass( 'ctrlbp-group-clone' ) ) {
			...
			var $dependentField = selectorCache.get( dependentFieldSelector ),
				isDependentFieldVisible = $dependentField.closest( '.ctrlbp-field' ).attr( 'data-visible' );
	...
	function getScope( $field, ignoreGroupClone ) {
		...
		if ( !ignoreGroupClone ) {
			var $groupClone = $field.closest( '.ctrlbp-group-clone' );
	...
	function applyVisible( $element ) {
		...
		var $field = $element.closest( '.ctrlbp-field' );
		if ( $field.length ) {
			$element = $field;
		}
		...
		$element.find( inputSelectors ).each( function () {
			var $this = $( this ),
				$field = $this.closest( '.ctrlbp-field.required' );
			if ( $field.length ) {
	...
	function getToggleType( $element ) {
		var $type = $element.closest( '.ctrlbp-meta-box' ).children( '.ctrlbp-toggle-type' );
	...
	function getWatchedElements() {
		...
		$( '.ctrlbp-conditions' ).each( function () {
	...
	// https://github.com/wpmetabox/ctrlbp/issues/1195.
	$( window ).on( 'load', function () {
		...
		$( document ).on( 'ctrlbp-blocks-edit-ready', function ( e ) {
			watch();
	...
	$(document).on('change keyup ctrlbp_change', inputSelectors, function(e){
		runConditionalLogic();
		watch();

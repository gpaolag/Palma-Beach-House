( function ( $, ctrlbp ) {
	...
	function transform() {
		...
		if ( options.ajax_data ) {
			...
			options.ajax.transport = function ( params, success, failure ) {
				...
				var actions = {
					'post'             : 'ctrlbp_get_posts',
					'taxonomy'         : 'ctrlbp_get_terms',
					'taxonomy_advanced': 'ctrlbp_get_terms',
					'user'             : 'ctrlbp_get_users'
				};
	...
	function init( e ) {
		$( e.target ).find( '.ctrlbp-select_advanced' ).each( transform );
	}

	ctrlbp.$document
		.on( 'ctrlbp_ready', init )
		.on( 'clone', '.ctrlbp-select_advanced', transform );
} )( jQuery, ctrlbp );

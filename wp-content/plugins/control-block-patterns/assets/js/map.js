( function ( $, document, window, google, ctrlbp, i18n ) {
	'use strict';
	...
	MapField.prototype = {
		...
		initDomElements: function () {
			this.$canvas = this.$container.find( '.ctrlbp-map-canvas' );
			this.canvas = this.$canvas[0];
			this.$coordinate = this.$container.find( '.ctrlbp-map' );
		...
		addListeners: function () {
			...
			$( window ).on( 'ctrlbp_map_refresh', refresh );

			// Refresh on meta box hide and show
			ctrlbp.$document.on( 'postbox-toggled', refresh );
			// Refresh on sorting meta boxes
			$( '.control-block-patterns-sortables' ).on( 'sortstop', refresh );
		},
		...
		autocomplete: function () {
			...
			// If Control Block Patterns Geo Location installed. Do not run autocomplete.
			if ( $( '.ctrlbp-geo-binding' ).length ) {
				var geocodeAddress = that.geocodeAddress.bind( that );
		...
		findAddressField: function( fieldName ) {
			...
			$address = this.$container.closest( '.ctrlbp-group-clone' ).find( 'input[name*="[' + fieldName + ']"]' );
			if ( $address.length ) {
				return $address;
			}

			// If map and address is inside a non-cloneable group.
			$address = this.$container.closest( '.ctrlbp-group-wrapper' ).find( 'input[name*="[' + fieldName + ']"]' );
			if ( $address.length ) {
				return $address;
			}
	...
	function init( e ) {
		$( e.target ).find( '.ctrlbp-map-field' ).each( createController );
	}

	function restart() {
		$( '.ctrlbp-map-field' ).each( createController );
	}

	ctrlbp.$document
		.on( 'ctrlbp_ready', init )
		.on( 'clone', '.ctrlbp-input', restart );
} )( jQuery, document, window, google, ctrlbp, CTRLBP_Map );

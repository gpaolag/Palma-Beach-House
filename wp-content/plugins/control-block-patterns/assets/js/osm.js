( function( $, L, ctrlbp, i18n ) {
	...
	OsmField.prototype = {
		...
		initDomElements: function () {
			this.$canvas = this.$container.find( '.ctrlbp-osm-canvas' );
			this.canvas = this.$canvas[0];
			this.$coordinate = this.$container.find( '.ctrlbp-osm' );
	...
	function init( e ) {
		$( e.target ).find( '.ctrlbp-osm-field' ).each( createController );
	}

	function restart() {
		$( '.ctrlbp-osm-field' ).each( createController );
	}

	ctrlbp.$document
		.on( 'ctrlbp_ready', init )
		.on( 'clone', '.ctrlbp-input', restart );
} )( jQuery, L, ctrlbp, CTRLBP_Osm );
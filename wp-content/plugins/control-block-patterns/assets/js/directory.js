...
/* global _cbpSettings, confirm, tb_position */
window.wp = window.wp || {};
...
themes.view.Appearance = wp.Backbone.View.extend({
	...
	el: '#wpbody-content .wrap .cbp-theme-browser',
...
themes.Collection = Backbone.Collection.extend({
	...
	apiCall: function( request, paginated ) {
		return wp.ajax.send( 'cbp-query-patterns', {
			data: {
				// Request data.
				request: _.extend({
					per_page: 78
...
themes.view.Theme = wp.Backbone.View.extend({
	...
	events: {
		...
		'click .install-theme': 'insertPattern',
		'click .update-message': 'updateTheme',
		'click .theme-actions .cpb-insert-pattern': 'insertPattern'
	},
	...
	insertPattern: function( event ) {
		...
		$( document ).on( 'cbp-pattern-install-success', function() {
			_this.model.set( { 'installed': true } );
		} );

		wp.ajax.send( 'cbp-insert-pattern', {
			data: {
				id: $target.data('id'),
			...
			success : function(response) {
				...
				setTimeout(function() {
					...
					if(response.success){	
						$target.closest('div').find('.cbp-pattern-preview').remove();				
						$target.replaceWith(response.data.button)
					}                           
				}, 500);
			},
		});
...
themes.view.Preview = themes.view.Details.extend({
	...
	events: {
		...
		'keyup': 'keyEvent',
		'click .cpb-insert-pattern': 'insertPattern'
	},
...
themes.view.Themes = wp.Backbone.View.extend({
	...
	className: 'themes wp-clearfix  pattern-grid',
		...
		if ( this.model.get( 'hasUpdate' ) ) {
			...
			if ( $card.find( '.updating-message' ).length ) {
				...
				$modal.find( '.notice-warning' )
					...
					.find( 'p' ).text( cbp.updates.l10n.updating );
...
themes.Router = Backbone.Router.extend({
	...
	routes: {
		'edit.php?post_type=ctrl_block_patterns&page=directory&pattern=:slug': 'pattern',
		'tedit.php?post_type=ctrl_block_patterns&page=directory&pattern=:query': 'search',
		'edit.php?post_type=ctrl_block_patterns&page=directory&s=:query': 'search',
		'edit.php?post_type=ctrl_block_patterns&page=directory': 'patterns',
		'': 'patterns'
	},

	baseUrl: function( url ) {
		return 'edit.php?post_type=ctrl_block_patterns&page=directory' + url;
	},
...
themes.InstallerRouter = Backbone.Router.extend({
	routes: {
		'edit.php?post_type=ctrl_block_patterns&page=directory&pattern=:slug': 'preview',
		'edit.php?post_type=ctrl_block_patterns&page=directory&browse=:sort': 'sort',
		'edit.php?post_type=ctrl_block_patterns&page=directory&search=:query': 'search',
		'edit.php?post_type=ctrl_block_patterns&page=directory': 'sort'
	},
...
themes.RunInstaller = {
	...
	routes: function() {
		...
		themes.router.on( 'route:sort', function( sort ) {
			if ( ! sort ) {
				sort = 'new';
				themes.router.navigate( themes.router.baseUrl( '&browse=new' ), { replace: true } );
			}
			self.view.sort( sort );
...
$( function() {
	...
	$( '.broken-themes .delete-theme' ).on( 'click', function() {
		return confirm( _cbpSettings.settings.confirmDelete );
	});
});

( function ( $, ctrlbp ) {
	...
	function toggleTree() {
		var $this = $( this ),
			val = $this.val(),
			$tree = $this.siblings( '.ctrlbp-select-tree' ),
	...
	function init( e ) {
		var $select = $( e.target ).find( '.ctrlbp-select-tree > select' );
	...
	ctrlbp.$document
		...
		.on( 'change', '.ctrlbp-select-tree > select', toggleTree )
		.on( 'clone', '.ctrlbp-select-tree > select', instantiateSelect2 );
} )( jQuery, ctrlbp );

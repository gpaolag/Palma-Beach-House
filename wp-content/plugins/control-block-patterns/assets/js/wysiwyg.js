( function( $, wp, window, ctrlbp ) {
	...
	function transform() {
		...
		var originalId = getOriginalId( this ),
			...
			customSettings = $this.closest( '.ctrlbp-input' ).find( '.ctrlbp-wysiwyg-id' ).data( 'options' );
	...
	function getOriginalId( el ) {
		return el.closest( '.ctrlbp-input' ).querySelector( '.ctrlbp-wysiwyg-id' ).dataset.id;
	}
	...
	function init( e ) {
		$( e.target ).find( '.ctrlbp-wysiwyg' ).each( transform );
	}
	...
	function addRequiredAttribute() {
		if ( this.classList.contains( 'ctrlbp-wysiwyg-required' ) ) {
	...
	ctrlbp.$document
		.on( 'ctrlbp_blocks_edit', init )
		.on( 'ctrlbp_init_editors', init )
		.on( 'clone', '.ctrlbp-wysiwyg', function() {
			/*

...
(function( $, wp, settings ) {
	...
	cbp.updates.l10n = window.wp.deprecateL10nObject( 'cbp.updates.l10n', cbp.updates.l10n, '5.5.0' );
	 ...
	 * @type {string}
	 */
	cbp.updates.ajaxNonce = settings.ajax_nonce;
	 ...
	 * @type {bool}
	 */
	cbp.updates.shouldRequestFilesystemCredentials = false;
	 ...
	 *                                                             Default 'false'.
	 */
	cbp.updates.filesystemCredentials = {
		ftp:       {
			host:           '',
			username:       '',
			password:       '',
	 ...
	 * @type {function}
	 */
	cbp.updates.adminNotice = wp.template( 'wp-updates-admin-notice' );
	 ...
	 * @type {jQuery}
	 */
	cbp.updates.$elToReturnFocusToFromCredentialsModal = undefined;
	 ...
	 *
	 */
	cbp.updates.addAdminNotice = function( data ) {
		var $notice = $( data.selector ),
			$headerEnd = $( '.wp-header-end' ),
			$adminNotice;

		delete data.selector;
		$adminNotice = cbp.updates.adminNotice( data );
	 ...
	 */
	cbp.updates.ajax = function( action, data ) {
		var options = {};

		if ( cbp.updates.ajaxLocked ) {
			cbp.updates.queue.push( {
				action: action,
				data:   data
			} );
		...
		options.data = _.extend( data, {
			action:          action,
			_ajax_nonce:     cbp.updates.ajaxNonce,
			//_fs_nonce:       cbp.updates.filesystemCredentials.fsNonce,
			//username:        cbp.updates.filesystemCredentials.ftp.username,
			//password:        cbp.updates.filesystemCredentials.ftp.password,
			//hostname:        cbp.updates.filesystemCredentials.ftp.hostname,
			//connection_type: cbp.updates.filesystemCredentials.ftp.connectionType,
			//public_key:      cbp.updates.filesystemCredentials.ssh.publicKey,
			//private_key:     cbp.updates.filesystemCredentials.ssh.privateKey
		} );

		console.log(options);

		return wp.ajax.send( options ).always( cbp.updates.ajaxAlways );
	};
	 ...
	 */
	cbp.updates.ajaxAlways = function( response ) {
		if ( ! response.errorCode || 'unable_to_connect_to_filesystem' !== response.errorCode ) {
			cbp.updates.ajaxLocked = false;
			cbp.updates.queueChecker();
		}
	 ...
	 * @since 4.7.0
	 */
	cbp.updates.refreshCount = function() {
	 ...
	 *                      Can be 'plugin', 'theme'.
	 */
	cbp.updates.decrementCount = function( type ) {
	 ...
	 * @param {updatePluginSuccess=} args.success Optional. Success callback. Default: cbp.updates.updatePluginSuccess
	 * @param {updatePluginError=}   args.error   Optional. Error callback. Default: cbp.updates.updatePluginError
	 * @return {$.promise} A jQuery promise that represents the request,
	 *                     decorated with an abort() method.
	 */
	cbp.updates.updatePlugin = function( args ) {
		var $updateRow, $card, $message, message,
			$adminBarUpdates = $( '#wp-admin-bar-updates' );

		args = _.extend( {
			success: cbp.updates.updatePluginSuccess,
			error: cbp.updates.updatePluginError
		}, args );
		...
		return cbp.updates.ajax( 'update-plugin', args );
	};
	 ...
	 */
	cbp.updates.updatePluginSuccess = function( response ) {
		...
		cbp.updates.decrementCount( 'plugin' );
	 ...
	 */
	cbp.updates.updatePluginError = function( response ) {
		var $card, $message, errorMessage,
			$adminBarUpdates = $( '#wp-admin-bar-updates' );

		if ( ! cbp.updates.isValidResponse( response, 'update' ) ) {
			return;
		}

		if ( cbp.updates.maybeHandleCredentialError( response, 'update-plugin' ) ) {
			return;
		}
		...
		if ( 'plugins' === pagenow || 'plugins-network' === pagenow ) {
			...
			$card = $( '.plugin-card-' + response.slug )
				...
				.append( cbp.updates.adminNotice( {
	 ...
	 * @param {installPluginSuccess=} args.success Optional. Success callback. Default: cbp.updates.installPluginSuccess
	 * @param {installPluginError=}   args.error   Optional. Error callback. Default: cbp.updates.installPluginError
	 * @return {$.promise} A jQuery promise that represents the request,
	 *                     decorated with an abort() method.
	 */
	cbp.updates.installPlugin = function( args ) {
		var $card    = $( '.plugin-card-' + args.slug ),
			$message = $card.find( '.install-now' );

		args = _.extend( {
			success: cbp.updates.installPluginSuccess,
			error: cbp.updates.installPluginError
		}, args );
		...
		return cbp.updates.ajax( 'install-plugin', args );
	};
	 ...
	 */
	cbp.updates.installPluginSuccess = function( response ) {
	 ...
	 * @param {string}  response.errorMessage The error that occurred.
	 */
	cbp.updates.installPluginError = function( response ) {
		var $card   = $( '.plugin-card-' + response.slug ),
			$button = $card.find( '.install-now' ),
			errorMessage;

		if ( ! cbp.updates.isValidResponse( response, 'install' ) ) {
			return;
		}

		if ( cbp.updates.maybeHandleCredentialError( response, 'install-plugin' ) ) {
			return;
		}
	 ...
	 * @param {string} response.activateUrl URL to activate the just installed plugin.
	 */
	cbp.updates.installImporterSuccess = function( response ) {
		cbp.updates.addAdminNotice( {
			id:        'install-success',
	 ...
	 * @param {string}  response.errorMessage The error that occurred.
	 */
	cbp.updates.installImporterError = function( response ) {
	 ...
	 * @param {deletePluginSuccess=} args.success Optional. Success callback. Default: cbp.updates.deletePluginSuccess
	 * @param {deletePluginError=}   args.error   Optional. Error callback. Default: cbp.updates.deletePluginError
	 * @return {$.promise} A jQuery promise that represents the request,
	 *                     decorated with an abort() method.
	 */
	cbp.updates.deletePlugin = function( args ) {
		var $link = $( '[data-plugin="' + args.plugin + '"]' ).find( '.row-actions a.delete' );

		args = _.extend( {
			success: cbp.updates.deletePluginSuccess,
			error: cbp.updates.deletePluginError
		}, args );
		...
		return cbp.updates.ajax( 'delete-plugin', args );
	};
	 ...
	 */
	cbp.updates.deletePluginSuccess = function( response ) {
	 ...
	 * @param {string}  response.errorMessage The error that occurred.
	 */
	cbp.updates.deletePluginError = function( response ) {
		var $plugin, $pluginUpdateRow,
			pluginUpdateRow  = wp.template( 'item-update-row' ),
			noticeContent    = cbp.updates.adminNotice( {
		...
		if ( ! cbp.updates.isValidResponse( response, 'delete' ) ) {
			return;
		}

		if ( cbp.updates.maybeHandleCredentialError( response, 'delete-plugin' ) ) {
			return;
		}
	 ...
	 * @param {updateThemeSuccess=} args.success Optional. Success callback. Default: cbp.updates.updateThemeSuccess
	 * @param {updateThemeError=}   args.error   Optional. Error callback. Default: cbp.updates.updateThemeError
	 * @return {$.promise} A jQuery promise that represents the request,
	 *                     decorated with an abort() method.
	 */
	cbp.updates.updateTheme = function( args ) {
		var $notice;

		args = _.extend( {
			success: cbp.updates.updateThemeSuccess,
			error: cbp.updates.updateThemeError
		}, args );
		...
		return cbp.updates.ajax( 'update-theme', args );
	};
	 ...
	 */
	cbp.updates.updateThemeSuccess = function( response ) {
		...
		cbp.updates.addAdminNotice( _.extend( { selector: $notice }, updatedMessage ) );
		wp.a11y.speak( __( 'Update completed successfully.' ) );

		cbp.updates.decrementCount( 'theme' );
		...
		if ( isModalOpen && 'customize' !== pagenow ) {
			$( '.theme-info .theme-author' ).after( cbp.updates.adminNotice( updatedMessage ) );
		}
	};
	 ...
	 * @param {string} response.errorMessage The error that occurred.
	 */
	cbp.updates.updateThemeError = function( response ) {
		...
		if ( cbp.updates.maybeHandleCredentialError( response, 'update-theme' ) ) {
			return;
		}
	 ...
	 * @param {Object}               args
	 * @param {string}               args.slug    Theme stylesheet.
	 * @param {installThemeSuccess=} args.success Optional. Success callback. Default: cbp.updates.installThemeSuccess
	 * @param {installThemeError=}   args.error   Optional. Error callback. Default: cbp.updates.installThemeError
	 * @return {$.promise} A jQuery promise that represents the request,
	 *                     decorated with an abort() method.
	 */
	cbp.updates.installTheme = function( args ) {
		var $message = $( '.theme-install[data-slug="' + args.slug + '"]' );

		args = _.extend( {
			success: cbp.updates.installThemeSuccess,
			error: cbp.updates.installThemeError
		}, args );
		...
		return cbp.updates.ajax( 'cbp-insert-pattern', args );
	};
	 ...
	 */
	cbp.updates.installThemeSuccess = function( response ) {
	 ...
	 * @param {string} response.errorMessage The error that occurred.
	 */
	cbp.updates.installThemeError = function( response ) {
		var $card, $button,
			errorMessage = sprintf(
				/* translators: %s: Error string for a failed installation. */
				__( 'Installation failed: %s' ),
			...
			$message     = cbp.updates.adminNotice( {
				className: 'update-message notice-error notice-alt',
				message:   errorMessage
			} );
		...
		if ( cbp.updates.maybeHandleCredentialError( response, 'cbp-insert-pattern' ) ) {
			return;
		}
	 ...
	 * @param {Object}              args
	 * @param {string}              args.slug    Theme stylesheet.
	 * @param {deleteThemeSuccess=} args.success Optional. Success callback. Default: cbp.updates.deleteThemeSuccess
	 * @param {deleteThemeError=}   args.error   Optional. Error callback. Default: cbp.updates.deleteThemeError
	 * @return {$.promise} A jQuery promise that represents the request,
	 *                     decorated with an abort() method.
	 */
	cbp.updates.deleteTheme = function( args ) {
		var $button;
		...
		args = _.extend( {
			success: cbp.updates.deleteThemeSuccess,
			error: cbp.updates.deleteThemeError
		}, args );
		...
		return cbp.updates.ajax( 'delete-theme', args );
	};
	 ...
	 */
	cbp.updates.deleteThemeSuccess = function( response ) {
	 ...
	 * @param {string} response.errorMessage The error that occurred.
	 */
	cbp.updates.deleteThemeError = function( response ) {
		...
		if ( cbp.updates.maybeHandleCredentialError( response, 'delete-theme' ) ) {
			return;
		}
	 ...
	 */
	cbp.updates._addCallbacks = function( data, action ) {
		if ( 'import' === pagenow && 'install-plugin' === action ) {
			data.success = cbp.updates.installImporterSuccess;
			data.error   = cbp.updates.installImporterError;
		}
	 ...
	 */
	cbp.updates.queueChecker = function() {
		var job;

		if ( cbp.updates.ajaxLocked || ! cbp.updates.queue.length ) {
			return;
		}

		job = cbp.updates.queue.shift();

		// Handle a queue job.
		switch ( job.action ) {
			case 'install-plugin':
				cbp.updates.installPlugin( job.data );
				break;

			case 'update-plugin':
				cbp.updates.updatePlugin( job.data );
				break;

			case 'delete-plugin':
				cbp.updates.deletePlugin( job.data );
				break;

			case 'cbp-insert-pattern':
				cbp.updates.installTheme( job.data );
				break;

			case 'update-theme':
				cbp.updates.updateTheme( job.data );
				break;

			case 'delete-theme':
				cbp.updates.deleteTheme( job.data );
				break;
	 ...
	 */
	cbp.updates.requestFilesystemCredentials = function( event ) {
		if ( false === cbp.updates.filesystemCredentials.available ) {
			/*
			 * After exiting the credentials request modal,
			 * return the focus to the element triggering the request.
			 */
			if ( event && ! cbp.updates.$elToReturnFocusToFromCredentialsModal ) {
				cbp.updates.$elToReturnFocusToFromCredentialsModal = $( event.target );
			}

			cbp.updates.ajaxLocked = true;
			cbp.updates.requestForCredentialsModalOpen();
		}
	};
	 ...
	 * @param {Event=} event Optional. Event interface.
	 */
	cbp.updates.maybeRequestFilesystemCredentials = function( event ) {
		if ( cbp.updates.shouldRequestFilesystemCredentials && ! cbp.updates.ajaxLocked ) {
			cbp.updates.requestFilesystemCredentials( event );
		}
	};
	 ...
	 */
	cbp.updates.keydown = function( event ) {
		if ( 27 === event.keyCode ) {
			cbp.updates.requestForCredentialsModalCancel();
	 ...
	 * @since 4.2.0
	 */
	cbp.updates.requestForCredentialsModalOpen = function() {
		var $modal = $( '#request-filesystem-credentials-dialog' );

		$( 'body' ).addClass( 'modal-open' );
		$modal.show();
		$modal.find( 'input:enabled:first' ).trigger( 'focus' );
		$modal.on( 'keydown', cbp.updates.keydown );
	};
	 ...
	 * @since 4.2.0
	 */
	cbp.updates.requestForCredentialsModalClose = function() {
		$( '#request-filesystem-credentials-dialog' ).hide();
		$( 'body' ).removeClass( 'modal-open' );

		if ( cbp.updates.$elToReturnFocusToFromCredentialsModal ) {
			cbp.updates.$elToReturnFocusToFromCredentialsModal.trigger( 'focus' );
		}
	};
	 ...
	 */
	cbp.updates.requestForCredentialsModalCancel = function() {
		...
		_.each( cbp.updates.queue, function( job ) {
		...
		cbp.updates.requestForCredentialsModalClose();
	};
	 ...
	 */
	cbp.updates.showErrorInCredentialsForm = function( message ) {
	 ...
	 * @param {string} action   The type of request to perform.
	 */
	cbp.updates.credentialError = function( response, action ) {

		// Restore callbacks.
		response = cbp.updates._addCallbacks( response, action );

		cbp.updates.queue.unshift( {
			action: action,
		...
		cbp.updates.filesystemCredentials.available = false;
		cbp.updates.showErrorInCredentialsForm( response.errorMessage );
		cbp.updates.requestFilesystemCredentials();
	};
	 ...
	 */
	cbp.updates.maybeHandleCredentialError = function( response, action ) {
		if ( cbp.updates.shouldRequestFilesystemCredentials && response.errorCode && 'unable_to_connect_to_filesystem' === response.errorCode ) {
			cbp.updates.credentialError( response, action );
			return true;
		}
	 ...
	 *                                                'update' or 'install'.
	 */
	cbp.updates.isValidResponse = function( response, action ) {
	 ...
	 * @since 4.2.0
	 */
	cbp.updates.beforeunload = function() {
		if ( cbp.updates.ajaxLocked ) {
	...
	$( function() {
		 ...
		 */
		cbp.updates.shouldRequestFilesystemCredentials = $filesystemModal.length > 0;
		...
		$filesystemModal.on( 'submit', 'form', function( event ) {
			...
			cbp.updates.filesystemCredentials.ftp.hostname       = $( '#hostname' ).val();
			cbp.updates.filesystemCredentials.ftp.username       = $( '#username' ).val();
			cbp.updates.filesystemCredentials.ftp.password       = $( '#password' ).val();
			cbp.updates.filesystemCredentials.ftp.connectionType = $( 'input[name="connection_type"]:checked' ).val();
			cbp.updates.filesystemCredentials.ssh.publicKey      = $( '#public_key' ).val();
			cbp.updates.filesystemCredentials.ssh.privateKey     = $( '#private_key' ).val();
			cbp.updates.filesystemCredentials.fsNonce            = $( '#_fs_nonce' ).val();
			cbp.updates.filesystemCredentials.available          = true;
		 ...
		 * @since 4.2.0
		 */
		$filesystemModal.on( 'click', '[data-js-action="close"], .notification-dialog-background', cbp.updates.requestForCredentialsModalCancel );
		...
		$bulkActionForm.on( 'click', '[data-plugin] .update-link', function( event ) {
			...
			cbp.updates.maybeRequestFilesystemCredentials( event );

			// Return the user to the input box of the plugin's table row after closing the modal.
			cbp.updates.$elToReturnFocusToFromCredentialsModal = $pluginRow.find( '.check-column input' );
			cbp.updates.updatePlugin( {
		...
		$document.on( 'click', '.importer-item .install-now', function( event ) {
			...
			cbp.updates.installPlugin( {
				slug:    $button.data( 'slug' ),
				pagenow: pagenow,
				success: cbp.updates.installImporterSuccess,
				error:   cbp.updates.installImporterError
			} );
		} );
		...
		$document.on( 'click', '.themes-php.network-admin .update-link', function( event ) {
			...
			cbp.updates.$elToReturnFocusToFromCredentialsModal = $themeRow.find( '.check-column input' );
			cbp.updates.updateTheme( {
		...
		$bulkActionForm.on( 'click', '[type="submit"]:not([name="clear-recent-list"])', function( event ) {
			...
			if ( ! itemsSelected.length ) {
				...
				return cbp.updates.addAdminNotice( {
			...
			$document.on( 'wp-plugin-update-success wp-plugin-update-error wp-theme-update-success wp-theme-update-error', function( event, response ) {
				...
				cbp.updates.adminNotice = wp.template( 'wp-bulk-updates-admin-notice' );
				...
				if ( error > 0 && ! cbp.updates.queue.length ) {
					$( 'html, body' ).animate( { scrollTop: 0 } );
				}
			} );
		...
		$pluginInstallSearch.on( 'keyup input', _.debounce( function( event, eventtype ) {
			...
			if ( cbp.updates.searchTerm === data.s && 'typechange' !== eventtype ) {
				return;
			} else {
				$pluginFilter.empty();
				cbp.updates.searchTerm = data.s;
			}
			...
			if ( 'undefined' !== typeof cbp.updates.searchRequest ) {
				cbp.updates.searchRequest.abort();
			}
			$( 'body' ).addClass( 'loading-content' );

			cbp.updates.searchRequest = wp.ajax.post( 'search-install-plugins', data ).done( function( response ) {
				$( 'body' ).removeClass( 'loading-content' );
				$pluginFilter.append( response.items );
				delete cbp.updates.searchRequest;
		...
		$pluginSearch.on( 'keyup input', _.debounce( function( event ) {
			...
			if ( cbp.updates.searchTerm === data.s ) {
				return;
			} else {
				cbp.updates.searchTerm = data.s;
			}
			...
			cbp.updates.searchRequest = wp.ajax.post( 'search-plugins', data ).done( function( response ) {
		...
		$( window ).on( 'message', function( event ) {
			...
			switch ( message.action ) {
				...
				case 'decrementUpdateCount':
					...
					cbp.updates.decrementCount( message.upgradeType );
					break;
				...
				case 'update-plugin':
					...
					message.data = cbp.updates._addCallbacks( message.data, message.action );

					cbp.updates.queue.push( message );
					cbp.updates.queueChecker();
					break;
			}
		} );
		 ...
		 * @since 4.2.0
		 */
		$( window ).on( 'beforeunload', cbp.updates.beforeunload );
